...
test("1-2. Header More Tools contains Urdu Writer once with canonical route", async () => {
  await renderHeader();
  const moreBtn = screen.getByRole("button", { name: /more tools/i });
  await act(async () => { fireEvent.click(moreBtn); });
  const items = screen.getAllByRole("menuitem").filter((a) =>
    (a as HTMLAnchorElement).href?.includes("/tools/roman-urdu-writer") ||
    (a.getAttribute("href") ?? "").includes("/tools/roman-urdu-writer")
  );
  expect(items).toHaveLength(1);
  expect(items[0].textContent).toMatch(/Urdu Writer/);
});
...
test("4-6. homepage How It Works card links correctly in English", async () => {
  ...
  const card = screen.getByRole("link", { name: /Qalam Urdu Writer/i });
  expect((card as HTMLAnchorElement).getAttribute("href")).toBe("/tools/roman-urdu-writer");
  expect(card.textContent).toMatch(/Write Urdu easily from Roman Urdu/);
...
test("7. Urdu How It Works card copy", async () => {
  mockLanguage = "ur";
  await renderHowItWorks();
  const card = screen.getByRole("link", { name: /قلم اردو رائٹر/ });
  expect((card as HTMLAnchorElement).getAttribute("href")).toBe("/tools/roman-urdu-writer");
  expect(card.textContent).toMatch(/رومن اردو سے آسانی سے اردو لکھیں/);
});

test("job guidance discovers Urdu Writer", async () => {
  await renderJobGuidance();
  const card = screen.getByRole("link", { name: /Qalam Urdu Writer|Roman/i });
  expect((card as HTMLAnchorElement).getAttribute("href")).toBe("/tools/roman-urdu-writer");
});
...
test("8-9. footer contains Urdu Writer once with canonical route", async () => {
  ...
  expect(links[0].textContent).toMatch(/Urdu Writer|قلم اردو رائٹر/);
});
...
test("12. Header primary nav does not include Urdu Writer", async () => {
  ...
  const primaryWriter = screen.queryAllByRole("link", { name: /^Urdu Writer$/i });
  expect(primaryWriter.length).toBe(0);
  const moreBtn = screen.getByRole("button", { name: /more tools/i });
  await act(async () => { fireEvent.click(moreBtn); });
  const items = screen.getAllByRole("menuitem").filter((el) =>
    (el.getAttribute("href") ?? "").includes("/tools/roman-urdu-writer")
  );
  expect(items).toHaveLength(1);
});
...
test("13-15. existing major tool links unchanged", async () => {
  ...
  expect(screen.getByRole("link", { name: /Translation Studio/i }).getAttribute("href")).toBe("/tools/translation-studio");
...
test("16-17. Urdu footer localization", async () => {
  mockLanguage = "ur";
  await renderFooter();
  const link = screen.getByRole("link", { name: /قلم اردو رائٹر/ });
  expect((link as HTMLAnchorElement).getAttribute("href")).toBe("/tools/roman-urdu-writer");
});
...
test("18-19. no accuracy or research terminology in discovery copy", () => {
  ...
  expect(src).toMatch(/Qalam Urdu Writer/);
  expect(src).not.toMatch(/Roman Urdu Converter|Perfect Urdu|100% accuracy|AI Urdu Converter/);
});
