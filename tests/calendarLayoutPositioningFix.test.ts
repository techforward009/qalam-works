...
describe("Calendar layout positioning fix", () => {
  ...
  it("centers weekday text inside a taller strip", () => {
    ...
    expect(CALENDAR_VISUAL_SPEC.print.weekdayStripPortraitMm).toBe(7.9);
    expect(CALENDAR_VISUAL_SPEC.print.weekdayStripLandscapeMm).toBe(7.4);
  ...
  it("roughly doubles central month-title size and prevents year clipping", () => {
    expect(CALENDAR_VISUAL_SPEC.print.monthTitleFontPortrait).toBe("30px");
    expect(CALENDAR_VISUAL_SPEC.print.monthTitleFontLandscape).toBe("28px");
