...
describe.runIf(Boolean(process.env.QALAM_PDF_CHROMIUM))("actual Document Studio PDF pipeline", () => {
  ...
  it("preserves structured blocks, hard breaks, blank spacing, and marked inline runs", async () => {
    ...
    const pages = JSON.parse(await import("node:fs").then(fs => fs.readFileSync(path.join(output, `${currentCase}-print-lines.json`), "utf8"))) as Array<Array<{ id: number; text: string; measuredInk: { left: number; right: number }; finalBounds: { top: number; bottom: number } }>>;
    ...
    const printableWidth = parseFloat(String(response.headers.get("x-pdf-printable-width") ?? 1e9));
    const physicalRight = printableWidth + 25.4 * 96 / 25.4 + 1;
    for (const line of pages.flat()) {
      expect(line.measuredInk.left).toBeGreaterThanOrEqual(-25.4 * 96 / 25.4 - 1);
      expect(line.measuredInk.right).toBeLessThanOrEqual(physicalRight);
    }
    const structure = JSON.parse(await import("node:fs").then(fs => fs.readFileSync(path.join(output, `${currentCase}-page-structure.json`), "utf8"))) as Array<{ semantic:{nestedOl:boolean;nestedUl:boolean;ordered:Array<{start:number;values:number[]}>;multiParagraphItem:boolean} }>;
  ...
  it("uses the same translation for final paint and diagnostics across alignment and direction", async () => {
    ...
    for (const line of lines) {
      ...
      expect(line.measuredInk.left).toBeGreaterThanOrEqual(-25.4 * 96 / 25.4 - 1);
      expect(line.measuredInk.right).toBeLessThanOrEqual(physicalRight);
      expect(line.measuredInk.horizontalShift).toBe(0);
