...
describe("Batch 17A.1 — DOCX source import via extractTextFromFile", () => {
  // DOCX fixture validity: uses mammoth Node { buffer } API (test-only) to
  // confirm the generated bytes are valid and extractable, then segmentation
  // produces rtl/ltr/rtl. The production browser path uses { arrayBuffer } —
  // not testable here without a real browser; the live Android/Chrome test is
  // the final authority for that path.
  test("DOCX bytes are valid: Node mammoth extracts Urdu + English, rtl/ltr/rtl directions", async () => {
    ...
    // Test-only: Node mammoth { buffer } to prove the fixture DOCX is valid.
