...
describe("historical crescent decision lookup", () => {
  ...
  it("excludes invalid and distinguishes reported records", () => {
    const base = PAKISTAN_OFFICIAL_HIJRI_ANCHORS[0];
  ...
  it("prefers matching verified official evidence independently of input order", () => {
    const official = PAKISTAN_OFFICIAL_HIJRI_ANCHORS[0], reported = { ...official, id: "reported", authority: "reported-official" as const, verificationStatus: "reported" as const };
  ...
  it("rejects conflicting same-evening decisions", () => {
    const official = PAKISTAN_OFFICIAL_HIJRI_ANCHORS[0];
  ...
  it("uses lexical IDs for equivalent same-precedence ties independent of array order", () => {
    const base = PAKISTAN_OFFICIAL_HIJRI_ANCHORS[0], z = { ...base, id: "z" }, a = { ...base, id: "a" };
