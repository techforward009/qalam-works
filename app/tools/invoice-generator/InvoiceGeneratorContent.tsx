  ...
  return (
    <main className="py-10 md:py-14" dir={dir}>
      ...
      <section className="site-container" dir={dir}>
        <h2 className={`text-lg font-bold text-gray-900 mb-4 ${language === "ur" ? "text-right font-nastaliq font-normal" : ""}`}>
          {t.faqHeading}
        </h2>
        <div className="space-y-4">
          {t.faqs.map((faq) => (
            <div key={faq.question} className="border border-gray-200 rounded-xl p-4">
              <p className={`font-semibold text-amber-900 mb-1 ${naskh}`}>{faq.question}</p>
