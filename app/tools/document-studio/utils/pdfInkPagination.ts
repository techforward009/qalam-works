...
export function fitHorizontalInk(
  ...
  if (frameLeft < -epsilon || frameRight > contentWidthPx + epsilon) {
    throw new Error(`PDF glyph ink cannot fit the printable horizontal boundary for line ${index} (${JSON.stringify(text)}): ${inkLeft}..${inkRight} in 0..${contentWidthPx}; export blocked`);
  }
  if (inkLeft < -leftMarginPx - epsilon || inkRight > contentWidthPx + rightMarginPx + epsilon) {
    throw new Error(`PDF glyph ink cannot fit the printable horizontal boundary for line ${index} (${JSON.stringify(text)}): ${inkLeft}..${inkRight} in 0..${contentWidthPx}; export blocked`);
  }
  return 0;
}
...
export async function paginatePdfInk(
  page: Page,
  widthMm: number,
  heightMm: number,
  leftMarginMm = 0,
  rightMarginMm = 0,
): Promise<{ lines: number; pages: number; stagingHeight: number; screenshots: number; measurementMs: number }> {
  const measurementStarted = Date.now();
  const width = mmToCssPx(widthMm), height = mmToCssPx(heightMm);
  const leftMarginPx = mmToCssPx(leftMarginMm), rightMarginPx = mmToCssPx(rightMarginMm);
  const horizontalOverscan = 64;
  ...
  lines.forEach((line, i) => {
    ...
    line.horizontalShift = fitHorizontalInk(
      line.inkLeft,
      line.inkRight,
      width,
      i,
