...
export default function CrescentVisibilityContent() {
  ...
  return <main className="mx-auto max-w-5xl px-4 py-8" dir={ur ? "rtl" : "ltr"}>
    ...
    <details className="mt-4"><summary className={`font-bold ${ur ? "font-naskh" : ""}`}>{ur ? "مزید سائنسی تفصیلات" : "Scientific details"}</summary><p className={`mt-2 ${ur ? "font-naskh" : ""}`}>{ur ? "سائنسی پیش گوئیاں سرکاری رویتِ ہلال کا اعلان نہیں ہیں۔" : "Scientific predictions are not official moon-sighting declarations."}</p><div className="mt-3 space-y-2">{pakistan.locations.map(({ observer, prediction }, index) => { const yallopPrediction = yallop.locations[index].prediction; return <div key={observer.id} className="rounded-lg border p-3 text-sm"><strong>{ur ? (URDU_LOCATION_NAMES[observer.id] ?? observer.name) : observer.name}</strong><p>{ur ? "پاکستانی معیار" : "Pakistan criterion"}: {prediction.status === "evaluated" && prediction.criterion.qualifies ? (ur ? "پورا" : "pass") : (ur ? "پورا نہیں" : "fail")}</p><p dir="ltr">Yallop: {yallopPrediction.status === "evaluated" ? `${yallopPrediction.criterion.visibilityClass} · q ${yallopPrediction.criterion.q.toFixed(3)}` : "unavailable"}</p></div>; })}</div></details>
  </main>;
}
