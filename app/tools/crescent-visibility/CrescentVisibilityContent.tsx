...
export default function CrescentVisibilityContent() {
  ...
  return <main className="mx-auto max-w-5xl px-4 py-8" dir={ur ? "rtl" : "ltr"}>
    ...
    <section className="mt-4 rounded-2xl border p-5"><h2 className={`font-bold ${ur ? "font-naskh" : ""}`}>{ur ? "پاکستان پانچ سالہ تقویمی معیار" : "Pakistan 5-Year Calendar Criterion"}</h2><p className={`mt-2 text-lg font-semibold ${ur ? "font-naskh" : ""}`}>{pakistan.qualifies ? (ur ? "پاکستان کے مقررہ قومی مشاہداتی مقامات میں کم از کم ایک مقام رؤیتِ ہلال کے سائنسی معیار پر پورا اترتا ہے۔" : "At least one of Pakistan's prescribed national observation locations meets the scientific crescent-visibility criterion.") : (ur ? "پاکستان کے مقررہ قومی مشاہداتی مقامات میں فی الحال کوئی مقام رؤیتِ ہلال کے سائنسی معیار پر پورا نہیں اترتا۔" : "None of Pakistan's prescribed national observation locations currently meets the scientific crescent-visibility criterion.")}</p></section>
    <section className="mt-4 rounded-2xl border p-5"><h2 className={`font-bold ${ur ? "font-naskh" : ""}`}>{ur ? "قومی حوالہ جاتی مقامات پر یالوپ تقابل" : "Yallop comparison across national reference locations"}</h2><p className={`mt-2 ${ur ? "font-naskh" : ""}`}>{yallop.anyAcceptedByPolicy ? (ur ? "کم از کم ایک قومی حوالہ جاتی مقام موجودہ قلم یالوپ A/B پالیسی تک پہنچتا ہے۔" : "At least one national reference location reaches the current Qalam Yallop A/B policy.") : (ur ? "کوئی قومی حوالہ جاتی مقام موجودہ قلم یالوپ A/B پالیسی تک نہیں پہنچتا۔" : "No national reference location reaches the current Qalam Yallop A/B policy.")}</p><p className="mt-2 text-sm text-muted-foreground" dir="ltr">{classSummary}</p></section>
    ...
    <details className="mt-4"><summary className={`font-bold ${ur ? "font-naskh" : ""}`}>{ur ? "مزید سائنسی تفصیلات" : "Scientific details"}</summary><p className={`mt-2 ${ur ? "font-naskh" : ""}`}>{ur ? "سائنسی پیش گوئیاں سرکاری رویتِ ہلال کا اعلان نہیں ہیں۔" : "Scientific predictions are not official moon-sighting declarations."}</p><div className="mt-3 space-y-2">{pakistan.locations.map(({ observer, prediction }, index) => { const yallopPrediction = yallop.locations[index].prediction; return <div key={observer.id} className="rounded-lg border p-3 text-sm"><strong>{observer.name}</strong><p>{ur ? "پاکستانی معیار" : "Pakistan criterion"}: {prediction.status === "evaluated" && prediction.criterion.qualifies ? (ur ? "پورا" : "pass") : (ur ? "پورا نہیں" : "fail")}</p><p dir="ltr">Yallop: {yallopPrediction.status === "evaluated" ? `${yallopPrediction.criterion.visibilityClass} · q ${yallopPrediction.criterion.q.toFixed(3)}` : "unavailable"}</p></div>; })}</div></details>
  </main>;
}
