...
export default function CrescentVisibilityContent() {
  ...
  const yallopPublicSummary = yallop.classCounts.A === 8
    ? (ur ? "تمام 8 حوالہ جاتی مقامات پر رؤیت کے حالات بہت موافق ہیں۔" : "Visibility conditions are very favorable across all 8 reference locations.")
    : yallopAcceptedCount === 8
      ? (ur ? "تمام 8 حوالہ جاتی مقامات پر رؤیت کے حالات موافق ہیں۔" : "Visibility conditions are favorable across all 8 reference locations.")
      : yallopAcceptedCount > 0
        ...
        : (ur ? "اس ماڈل کے مطابق 8 میں سے کسی حوالہ جاتی مقام پر رؤیت کے حالات موافق نہیں ہیں۔" : "None of the 8 reference locations currently have favorable visibility conditions under this model.");
  ...
  return <main className="mx-auto max-w-5xl px-4 py-10" dir={ur ? "rtl" : "ltr"}>
    <header><h1 className={`text-4xl font-bold tracking-tight text-[#1A3A2A] dark:text-white ${ur ? "font-naskh" : ""}`}>{ur ? "پاکستان میں رؤیتِ ہلال" : "Pakistan Crescent Visibility"}</h1><p className={`mt-2 text-lg text-muted-foreground ${ur ? "font-naskh" : ""}`}>{ur ? "قومی سائنسی اور سرکاری جائزہ" : "National scientific and official outlook"}</p></header>
    <section className={`mt-4 text-sm text-muted-foreground ${ur ? "font-naskh" : ""}`}>{ur ? "سرکاری فیصلہ پورے پاکستان سے موصول ہونے والی معتبر اور قابلِ قبول شہادتوں کی بنیاد پر کیا جاتا ہے۔ یہ صفحہ مقررہ قومی مقامات کے فلکیاتی حالات کا سائنسی جائزہ پیش کرتا ہے۔" : "Pakistan's official decision is based on credible accepted testimony from across the country. This page presents a scientific outlook based on prescribed national reference locations."}</section>
    {dateControl}
    <section className="mt-5 rounded-[22px] bg-[#F7F5EF] p-6 shadow-sm dark:bg-[#162a1e]">
      <p className="text-sm font-semibold text-[#335a3d] dark:text-[#d7eadb]">{formatDate(date, "gregorian", lang)}</p>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2"><div className="rounded-xl bg-white/70 p-4 dark:bg-black/10"><h2 className={`font-semibold ${ur ? "font-naskh" : ""}`}>{ur ? "پاکستانی قومی سائنسی معیار" : "Pakistan national scientific criterion"}</h2><p className={`mt-1 ${ur ? "font-naskh" : ""}`}>{pakistanSummary}</p></div><div className="rounded-xl bg-white/70 p-4 dark:bg-black/10"><h2 className={`font-semibold ${ur ? "font-naskh" : ""}`}>{ur ? "بین الاقوامی طور پر معروف سائنسی ماڈل" : "International crescent-visibility model"}</h2><p className={`mt-1 ${ur ? "font-naskh" : ""}`}>{yallopPublicSummary}</p><p className={`mt-2 text-xs text-muted-foreground ${ur ? "font-naskh" : ""}`}>{ur ? "استعمال شدہ ماڈل: Yallop" : "Model used: Yallop"}</p></div></div>
    </section>
    ...
    <section className="mt-5 grid gap-4 md:grid-cols-3"><article className="rounded-[20px] border p-5"><h2 className={`font-bold ${ur ? "font-naskh" : ""}`}>{ur ? "پاکستان کی سرکاری حیثیت" : "Official Pakistan status"}</h2><p className={`mt-2 ${ur ? "font-naskh" : ""}`}>{authority ? formatDate(authority.hijri, "hijri", lang) : (ur ? "اس تاریخ کے لیے پاکستان کی سرکاری ہجری تاریخ دستیاب نہیں۔" : "Official Pakistan Hijri date is not available for this date.")}</p></article><article className="rounded-[20px] border p-5"><h2 className={`font-bold ${ur ? "font-naskh" : ""}`}>{ur ? "پاکستانی قومی سائنسی معیار" : "Pakistan national scientific criterion"}</h2><p className={`mt-2 ${ur ? "font-naskh" : ""}`}>{pakistanSummary}</p><p className={`mt-3 text-xs text-muted-foreground ${ur ? "font-naskh" : ""}`}>{ur ? "یہ سائنسی نتیجہ ہے، سرکاری رویتِ ہلال کا اعلان نہیں۔" : "Scientific result — not an official moon-sighting declaration."}</p></article><article className="rounded-[20px] border p-5"><h2 className={`font-bold ${ur ? "font-naskh" : ""}`}>{ur ? "بین الاقوامی طور پر معروف سائنسی ماڈل" : "International crescent-visibility model"}</h2><p className={`mt-2 ${ur ? "font-naskh" : ""}`}>{yallopPublicSummary}</p><p className="mt-3 text-xs text-muted-foreground">{ur ? "استعمال شدہ ماڈل: Yallop" : "Model used: Yallop"}</p></article></section>
    <section className="mt-4"><button type="button" aria-expanded={isYallopHelpOpen} aria-controls="yallop-class-help" onClick={() => setYallopHelpOpen(open => !open)} className={`text-sm font-semibold underline focus-visible:outline focus-visible:outline-2 ${ur ? "font-naskh" : ""}`}>{ur ? "یالوپ ماڈل کی وضاحت" : "About the Yallop model"}</button>{isYallopHelpOpen && <div id="yallop-class-help" className={`mt-2 rounded-xl bg-muted p-4 text-sm ${ur ? "font-naskh" : ""}`}><p>{ur ? "یہ ایک بین الاقوامی طور پر معروف فلکیاتی ماڈل ہے جو ہلال کی رؤیت کے امکان کو فلکیاتی اعداد کی بنیاد پر درجہ بند کرتا ہے۔ یہ سرکاری رویتِ ہلال کا اعلان نہیں۔" : "This is a widely used astronomical crescent-visibility model that classifies visibility potential from astronomical parameters. It is not an official moon-sighting declaration."}</p><p className="mt-2 text-xs">A: easily visible; B: visible under favorable conditions; C: may require optical aid; D: optical aid generally required; E/F: not expected to be visible.</p></div>}</section>
    {month && authority && (authority.hijri.day === 29 || authority.hijri.day === 30) && <section className="mt-4 rounded-xl border p-4"><h2 className={`font-bold ${ur ? "font-naskh" : ""}`}>{ur ? "آغازِ ماہ کا امکان" : "Month-start implication"}</h2><p className={ur ? "font-naskh" : ""}>{month.state === "day30_forced_next_month" ? (ur ? "سرکاری قمری سیاق کے مطابق کل نئے مہینے کی پہلی تاریخ ہے۔" : "Authoritative calendar context makes tomorrow the first day of the next month.") : month.state === "day29_qualifies" ? (ur ? "سائنسی نتائج کے مطابق کل آغازِ ماہ ہو سکتا ہے؛ حتمی فیصلہ سرکاری ہوگا۔" : "Scientific visibility may support next-day month start; an official decision remains required.") : (ur ? "کوئی سرکاری آغازِ ماہ کا نتیجہ اخذ نہیں کیا گیا۔" : "No official month-start conclusion is inferred.")}</p></section>}
    {!decision && <NationalEvidencePanel decision={decision} lang={lang} />}
    <details className="mt-6"><summary className={`font-bold ${ur ? "font-naskh" : ""}`}>{ur ? "مزید سائنسی تفصیلات" : "Scientific details"}</summary><p className={`mt-2 ${ur ? "font-naskh" : ""}`}>{ur ? "سائنسی پیش گوئیاں سرکاری رویتِ ہلال کا اعلان نہیں ہیں۔" : "Scientific predictions are not official moon-sighting declarations."}</p><div className="mt-3 grid gap-2 md:grid-cols-2" data-testid="scientific-location-grid">{pakistan.locations.map(({ observer, prediction }, index) => { const yallopPrediction = yallop.locations[index].prediction; const evaluatedPakistan = prediction.status === "evaluated"; const evaluatedYallop = yallopPrediction.status === "evaluated"; const passes = evaluatedPakistan && prediction.criterion.qualifies; return <div key={observer.id} className="rounded-lg border p-3 text-sm"><div className="flex items-center justify-between gap-2"><strong>{ur ? (URDU_LOCATION_NAMES[observer.id] ?? observer.name) : observer.name}</strong><span className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${passes ? "border-emerald-300 bg-emerald-100 text-emerald-900 dark:border-emerald-700 dark:bg-emerald-950 dark:text-emerald-100" : "border-rose-300 bg-rose-100 text-rose-900 dark:border-rose-700 dark:bg-rose-950 dark:text-rose-100"}`}>{passes ? (ur ? "معیار پورا" : "Pass") : (ur ? "معیار پورا نہیں" : "Fail")}</span></div>{evaluatedPakistan && <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1"><dt>{ur ? "ارتفاع" : "Altitude"}</dt><dd><span dir="ltr">{prediction.criterion.altitudeDeg.toFixed(2)}°</span></dd><dt>{ur ? "ہلال کی چوڑائی" : "Crescent width"}</dt><dd><span dir="ltr">{prediction.criterion.widthArcMin.toFixed(3)}′</span></dd><dt>{ur ? "روشن حصہ" : "Illumination"}</dt><dd><span dir="ltr">{prediction.criterion.illuminationPercent.toFixed(2)}%</span></dd><dt>{ur ? "زاویائی فاصلہ" : "Elongation"}</dt><dd><span dir="ltr">{prediction.criterion.elongationDeg.toFixed(2)}°</span></dd><dt>{ur ? "غروب کا وقفہ" : "Lag"}</dt><dd><span dir="ltr">{prediction.criterion.lagMinutes.toFixed(1)} min</span></dd></dl>}<div className="mt-2 flex items-center gap-2"><span>{ur ? "یالوپ" : "Yallop"}:</span>{evaluatedYallop ? <><span className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${classTone(yallopPrediction.criterion.visibilityClass)}`}>{ur ? "درجہ " : "Class "}{yallopPrediction.criterion.visibilityClass}</span><span dir="ltr">q = {qText(yallopPrediction.criterion.q)}</span></> : <span>{ur ? "دستیاب نہیں" : "unavailable"}</span>}</div></div>; })}</div></details>
  </main>;
}
