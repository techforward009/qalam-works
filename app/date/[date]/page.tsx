...
export default async function DateProfilePage({params,searchParams}:{params:Promise<{date:string}>,searchParams?:Promise<{countryId?:string}>}) {
 const {date}=await params;
 const parsed=parseValidDate(date);
 if(!parsed) notFound();
 const countryId = searchParams ? (await searchParams)?.countryId : undefined;
 return <main dir="auto" className="mx-auto max-w-3xl p-6"><DateProfileCard profile={buildDateProfile(parsed,countryId)}/></main>;
}
